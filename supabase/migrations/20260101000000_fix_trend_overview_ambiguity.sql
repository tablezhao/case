-- 修复 RPC 函数重载导致的歧义
-- 删除旧的、可能导致冲突的函数签名
DROP FUNCTION IF EXISTS get_trend_overview(integer, integer, text);
DROP FUNCTION IF EXISTS get_trend_overview(integer, integer);
      ...
      'high_frequency_months', (
        ...
        FROM (
          SELECT
            ...
            COUNT(DISTINCT department_id || '_' || report_date) AS count
          FROM cases
        ...
        'monthly', (
          ...
          FROM (
            SELECT
              ...
              COUNT(DISTINCT c.department_id || '_' || c.report_date) AS count
            FROM cases c
