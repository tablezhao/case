...
import { deleteFrontendConfigByModuleKey, getFrontendConfigs, updateFrontendConfig } from '@/db/api';
...
export default function HomeConfigPage() {
  ...
  const loadConfigs = async () => {
    try {
      ...
      const moduleKeysToRemove = ['wordcloud', 'trend_chart'];
      await Promise.all(
        moduleKeysToRemove
          .filter((moduleKey) => data.some((config) => config.module_key === moduleKey))
          .map((moduleKey) => deleteFrontendConfigByModuleKey(moduleKey).catch(() => undefined))
      );
      setConfigs(data.filter((config) => !moduleKeysToRemove.includes(config.module_key)));
    } catch (error: any) {
      toast.error('加载失败', {
